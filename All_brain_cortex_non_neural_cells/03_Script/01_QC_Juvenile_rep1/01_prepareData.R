...
## @knitr loadData
sc10x.rna = read.table(file.path(PATH_MOUSE_RNA_COUNT_TABLE, paste0(REPLICATE_NAME, "_gene_counts.tsv")), sep = "\t", header = TRUE)
sc10x.rna = dcast(sc10x.rna, formula = gene~cell, value.var = "count")
rownames(sc10x.rna) <- sc10x.rna[,"gene"]
sc10x.rna <- select(sc10x.rna, -gene)
sc10x.rna[is.na(sc10x.rna)] <- 0


sc10x.rna.seurat = CreateSeuratObject( counts = sc10x.rna, 
