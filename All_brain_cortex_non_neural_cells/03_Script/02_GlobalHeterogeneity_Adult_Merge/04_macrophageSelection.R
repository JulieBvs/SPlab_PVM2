...
cl_interest <- which.max(AverageExpression(sc10x.rna.seurat.ad.all, features = "Lyve1")$RNA) - 1

#Extract cell barcodes from cluster of interest
##Extract barcodes
cell_bc_interest <- names(Idents(sc10x.rna.seurat.ad.all)[which(Idents(sc10x.rna.seurat.ad.all) == cl_interest)])
...
sc10x.rna.seurat.ad.subset <- subset(x = sc10x.rna.seurat.ad.all, idents = cl_interest)
