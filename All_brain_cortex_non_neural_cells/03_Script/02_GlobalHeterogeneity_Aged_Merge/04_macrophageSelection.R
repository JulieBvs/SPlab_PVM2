...
cl_interest <- which.max(AverageExpression(sc10x.rna.seurat.ag.all, features = "Lyve1")$RNA) - 1

#Extract cell barcodes from cluster of interest
##Extract barcodes
cell_bc_interest <- names(Idents(sc10x.rna.seurat.ag.all)[which(Idents(sc10x.rna.seurat.ag.all) == cl_interest)])
...
sc10x.rna.seurat.ag.subset <- subset(x = sc10x.rna.seurat.ag.all, idents = cl_interest)
#saveRDS( sc10x.rna.seurat.ag.all, file = file.path( PATH_ANALYSIS_OUTPUT, paste0( outputFilesPrefix, "sc10x.rna.seurat.cl", cl_interest ,".RDS")))
cat("<BR><BR>Number of selected cells: <BR>", ncol(GetAssayData(sc10x.rna.seurat.ag.all, slot = 'counts')))
