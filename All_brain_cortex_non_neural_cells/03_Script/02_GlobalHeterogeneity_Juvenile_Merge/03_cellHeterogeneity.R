...
# Run PCA
sc10x.rna.seurat.ju.all <- RunPCA( object   = sc10x.rna.seurat.ju.all, 
                                   features = VariableFeatures( sc10x.rna.seurat.ju.all),
                                   npcs     = PCA_NPC,
                                   verbose  = .VERBOSE)

# Run T-sne and UMAP
sc10x.rna.seurat.ju.all = RunTSNE( sc10x.rna.seurat.ju.all, dims = 1:DIMREDUC_USE_PCA_NBDIMS, seed.use = 1511);
sc10x.rna.seurat.ju.all = RunUMAP( sc10x.rna.seurat.ju.all, dims = 1:DIMREDUC_USE_PCA_NBDIMS, seed.use = 1511);
...
print( suppressWarnings( DimPlot( sc10x.rna.seurat.ju.all, reduction = ifelse(exists("useReduction"), useReduction, "umap"), group.by = "sample")) + 
         theme( axis.title.x = element_blank(),
                axis.title.y = element_blank(),
                plot.margin = margin(0, 0, 0, 0, "cm")))

print( suppressWarnings( DimPlot( sc10x.rna.seurat.ju.all, reduction = ifelse(exists("useReduction"), useReduction, "umap"), split.by = "sample", group.by = "sample")) + 
         theme( axis.title.x = element_blank(),
                axis.title.y = element_blank(),
...
sc10x.rna.seurat.ju.all <- FindNeighbors(sc10x.rna.seurat.ju.all, reduction = "pca", dims = 1:N_PCA)
sc10x.rna.seurat.ju.all <- FindClusters(sc10x.rna.seurat.ju.all, resolution = FINDCLUSTERS_RESOLUTION)


# Plot the projection with colored clusters
## @knitr cellClustering_visualization
print( suppressWarnings( DimPlot( sc10x.rna.seurat.ju.all, reduction = ifelse(exists("useReduction"), useReduction, "umap"), 
                                  label = FALSE, cols = col_rainbow[1:nlevels( Idents( sc10x.rna.seurat.ju.all))])) + 
         theme( axis.title.x = element_blank(),
                axis.title.y = element_blank(),
                plot.margin = margin(0, 0, 0, 0, "cm")))

print( suppressWarnings( DimPlot( sc10x.rna.seurat.ju.all, reduction = ifelse(exists("useReduction"), useReduction, "umap"), split.by = "sample", cols = col_rainbow[1:nlevels( Idents( sc10x.rna.seurat.ju.all))])) + 
         theme( axis.title.x = element_blank(),
                axis.title.y = element_blank(),
                legend.position = "none", 
                plot.margin = margin(0, 0, 0, 0, "cm")) +
...
markers = FindAllMarkers( object          = sc10x.rna.seurat.ju.all,
...
#htmltools::tagList( ggplotly( DoHeatmap(object = sc10x.rna.seurat.ju.all, features = topMarkersDF[["gene"]])));

# Get the matrix of expression and associated clusters from Seurat object 
expMat = as.matrix( GetAssayData( sc10x.rna.seurat.ju.all));
clusterID = Idents( sc10x.rna.seurat.ju.all);
...
hoverTextMarkers[] = paste( paste( "Cell ID:", sc10x.rna.seurat.ju.all[["numID", drop = FALSE]][colnames( topMarkersGenes_expMat)[col( topMarkersGenes_expMat)],]),
...
heatmapPlot = iheatmap(topMarkersGenes_expMat, 
                       ...
                       row_annotation_colors = list(Cluster = hue_pal()(nlevels(Idents( sc10x.rna.seurat.ju.all)))),
                       col_annotation_colors = list(Cluster = hue_pal()(nlevels(Idents( sc10x.rna.seurat.ju.all)))));
...
resNULL=lapply(names(topMarkers), function(clusterName)
{
  ...
  clusterCells = which( Idents( sc10x.rna.seurat.ju.all) == clusterName);
  # Create a dimreduc plot with current cluster highlighted (useful for large number of clusters)
  print( DimPlot( sc10x.rna.seurat.ju.all, 
  ...
  { 
    FeaturePlot( sc10x.rna.seurat.ju.all, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE, cols = col_greyMagma) +
      theme( axis.title.x = element_blank(),
             axis.title.y = element_blank(),
             legend.position = "none");
...
cellsDataMarkers = cbind( "Cell" = colnames( sc10x.rna.seurat.ju.all), # cell names from rownames conflict with search field in datatable
                          sc10x.rna.seurat.ju.all[[c( "numID", "nCount_RNA", "nFeature_RNA")]],
                          "percent.mito" = as.numeric(format(sc10x.rna.seurat.ju.all[["percent.mito", drop = TRUE]], digits = 5)),
                          "Cluster" = Idents( sc10x.rna.seurat.ju.all));

# # Add gene expression information on cell data
original_names = names( cellsDataMarkers)
for( marker_gene in topMarkersGenes){
  cellsDataMarkers = cbind( cellsDataMarkers, sc10x.rna.seurat.ju.all@assays$RNA@data[ marker_gene, colnames( sc10x.rna.seurat.ju.all)])
}
...
# DoHeatmap replaced by use of iHeatmapr after testing several options
#htmltools::tagList( ggplotly( DoHeatmap(object = sc10x.rna.seurat.ju.all, features = unlist(MARKERS_PVM))));
...
hoverTextMarkers[] = paste( paste( "Cell ID:", sc10x.rna.seurat.ju.all[["numID", drop = FALSE]][colnames( expMat)[col( expMat)],]),
                            paste( "Value:", expMat),
...
resNULL = lapply( names( MARKERS_ALL), function(monitoredGroup)
{
  ...
  {
    tryCatch( FeaturePlot( sc10x.rna.seurat.ju.all, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE, cols = col_greyMagma)  +
                theme( axis.title.x = element_blank(),
                       axis.title.y = element_blank(),
                       legend.position = "none"),
...
# Gather data to be visualized together (cell name + numID + metrics + Cluster)
cellsDataMonitored = cbind( "Cell" = colnames( sc10x.rna.seurat.ju.all), # cell names from rownames conflict with search field in datatable
                            sc10x.rna.seurat.ju.all[[c( "numID", "nCount_RNA", "nFeature_RNA")]],
                            "percent.mito" = as.numeric(format(sc10x.rna.seurat.ju.all[["percent.mito", drop = TRUE]], digits = 5)),
                            "Cluster" = Idents( sc10x.rna.seurat.ju.all));
...
for( monitored_gene in all_monitored_genes){
  cellsDataMonitored = cbind( cellsDataMonitored, sc10x.rna.seurat.ju.all@assays$RNA@data[ monitored_gene, colnames( sc10x.rna.seurat.ju.all)])
}
...
monitored_genes_dist <- as.dist(1 - cor(t(as.matrix(GetAssayData(sc10x.rna.seurat.ju.all, slot = "data")[unlist(MARKERS_ALL),], method = "pearson"))))
...
DotPlot(sc10x.rna.seurat.ju.all, features = monitored_genes_ordered, cols = c("blue", "orange", "red"), dot.scale = 8) +
  RotatedAxis() +
  coord_flip()

DotPlot(sc10x.rna.seurat.ju.all, features = monitored_genes_ordered, cols = c("blue", "orange", "red"), dot.scale = 8, cluster.idents = TRUE) +
  RotatedAxis() +
  coord_flip()
...
for (i in MARKERS_ALL$ALL) {
  print(plot_density(object = sc10x.rna.seurat.ju.all, features = i, reduction = "umap", size = 0.5))
}
...
## @knitr save_data
# saveRDS(sc10x.rna.seurat.ju.all, file = file.path(PATH_ANALYSIS_OUTPUT, paste0( outputFilesPrefix, "sc10x.rna.seurat.ju.all.RDS")))
# print(paste0("Exporting combined seurat object to : ", file.path(PATH_ANALYSIS_OUTPUT, paste0( outputFilesPrefix, "sc10x.rna.seurat.ju.all.RDS"))))
