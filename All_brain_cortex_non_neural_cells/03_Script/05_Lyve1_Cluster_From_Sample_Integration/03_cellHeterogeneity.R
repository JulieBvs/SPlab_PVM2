...
# for( monitoredGroup in names( MARKERS_PVM)){
#   sc10x <- AddModuleScore( object = sc10x, 
#                            features = MARKERS_PVM[[ monitoredGroup]],
#                            ctrl = length(MARKERS_PVM[[ monitoredGroup]]), 
#                            name = monitoredGroup);
# }
...
# #Generate a table of the mean of genes expression by cluster
# gene_exp <- t(as.data.frame(GetAssayData(object = sc10x.rna.seurat, slot = 'scale.data')))
# mean_gene_exp <- do.call(cbind, by(gene_exp, Idents(sc10x.rna.seurat), colMeans))
# write.table(mean_gene_exp, file=paste(PATH_ANALYSIS_OUTPUT, paste(SCIENTIFIC_PROJECT_NAME, EXPERIMENT_PROJECT_NAME, "gene_expression_by_cluster.csv",sep="_"), sep="/"), sep="\t", dec=".", col.names = NA)
...
resNULL=lapply(names(topMarkers), function(clusterName)
{
  ...
  { 
    FeaturePlot( sc10x.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE, cols = col_greyMagma) +
      theme( axis.title.x = element_blank(),
             axis.title.y = element_blank(),
             legend.position = "none");
...
# DoHeatmap replaced by use of iHeatmapr after testing several options
#htmltools::tagList( ggplotly( DoHeatmap(object = sc10x.rna.seurat, features = unlist(MARKERS_PVM))));
...
MARKERS_PVM <- list( ALL = MARKERS_PVM)

# Plot expression values of monitored genes (TODO: message if list empty)
resNULL = lapply( names( MARKERS_PVM), function(monitoredGroup)
{
  cat("######", monitoredGroup, " \n");
  
  # List of plots (or error message if feature not found)
  expressionPlots = lapply( MARKERS_PVM[[monitoredGroup]], function(featuresName)
  {
    tryCatch( FeaturePlot( sc10x.rna.seurat, features = featuresName, reduction = ifelse(exists("useReduction"), useReduction, "umap"), order = TRUE, cols = col_greyMagma)  +
                theme( axis.title.x = element_blank(),
                       axis.title.y = element_blank(),
                       legend.position = "none"),
...
DotPlot(sc10x.rna.seurat, features = c(MARKERS_FOS_COMPLEX, "Ptprc", "Lyve1", "Cx3cr1"), cols = c("blue", "orange", "red"), dot.scale = 8) +
  RotatedAxis() +
  coord_flip()

DotPlot(sc10x.rna.seurat, features = c("Ptprc", "Lyve1", "Cx3cr1", MARKERS_FOS_COMPLEX), cols = c("blue", "orange", "red"), dot.scale = 8) +
  RotatedAxis()

DotPlot(sc10x.rna.seurat, features = MARKERS_PVM[["ALL"]], cols = c("blue", "orange", "red"), dot.scale = 8) +
  RotatedAxis()

DotPlot(sc10x.rna.seurat, features = c("Nfkbia", "Nfkbid", MARKERS_FOS_COMPLEX, "Ptprc", "Lyve1", "Cx3cr1"), cols = c("blue", "orange", "red"), dot.scale = 8) +
  RotatedAxis() +
  coord_flip()
